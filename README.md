...
| **数据管理** | QMT 数据下载、缓存管理、数据质量检查、财务快照拉取（供因子研究注入基本面）、数据时效检查 |
...
- **回测节点**：向量化回测，默认成本佣金率 0.001 / 滑点 0.001 / 卖出印花税 0.0005（与回测接口、QUBE 一致），T 日信号 T+1 执行，输出净值/回撤曲线与完整绩效（年化收益/波动/夏普/索提诺/卡玛/最大回撤/VaR/CVaR/胜率/盈亏比/月度收益），支持可选基准对比（跟踪误差/信息比率）。逐仓风控（止盈/止损/移动止损）对**多头与空头**同时生效；一字板导致调仓无法成交时不推演挂单（持仓保持，次数在 assumptions 明示）。
...
- **复权限制**：行情为**前复权**，且是增量缓存；跨除权除息事件追更时会触发重算，但历史缓存不自动回写前复权价——如需跨大除权区间的精确面板，建议重新下载该区间（此限制明示，不作错误修正）。
