...
| 模块 | 说明 |
|------|------|
| **工作流编辑器** | iframe 内嵌官方 ComfyUI 前端，搭建研究管线；节点右键查看/编辑代码（Monaco + AI 改写 + ruff 诊断）、因子分析节点「显示分析结果」直接弹出综合报告、内置节点代码 fork 保护、底部本机性能监控；**节点完整输出落本地 pkl、运行记录只存预览**（`/api/workflow/runs/{run_id}/nodes/{node_uuid}/output` 按需取明细）；运行产物默认保留 30 天，启动时自动清理，也可 `POST /api/workflow/runs/cleanup` |
| **数据探索** | 数据概览、SQL 查询（含 AI 生成 SQL、结果解读、**SQL 历史/收藏**）、全市场扫描、横截面分析、异常检测，全部基于本地 Parquet 缓存；**市场环境仪表盘**（宽基牛/震荡/熊判定 + 小盘/成长/中盘 vs 大盘风格轮动，数据中心顶部）；**事件研究**（一字涨/跌停、放量、除权除息、手工事件 → 窗口 CAR/BHAR + t 值 + 正值占比；支持指定基准指数与市场模型 alpha/beta，附加按事件日聚类的稳健 t 值；近邻事件自动合并防重复计数） |
