...
| 模块 | 说明 |
|------|------|
| **工作流编辑器** | iframe 内嵌官方 ComfyUI 前端，搭建研究管线；节点右键查看/编辑代码（Monaco + AI 改写 + ruff 诊断）、因子分析节点「显示分析结果」直接弹出综合报告、内置节点代码 fork 保护、底部本机性能监控；**节点完整输出落本地 pkl、运行记录只存预览**（`/api/workflow/runs/{run_id}/nodes/{node_uuid}/output` 按需取明细）；运行产物长期保留；如需清理，显式调用 `POST /api/workflow/runs/cleanup`（默认 dry_run=true 只统计，传 dry_run=false 才实际删除） |
| **数据探索** | 数据概览、SQL 查询（含 AI 生成 SQL、结果解读、**SQL 历史/收藏与字段字典/查询模板**）、全市场扫描、横截面分析、异常检测，全部基于本地 Parquet 缓存；**市场环境仪表盘**（宽基牛/震荡/熊判定 + 小盘/成长/中盘 vs 大盘风格轮动，数据中心顶部）；**事件研究**（一字涨/跌停、放量、除权除息、手工事件 → 窗口 CAR/BHAR + t 值 + 正值占比；支持指定基准指数与市场模型 alpha/beta，附加按事件日聚类的稳健 t 值，并提供 **calendar-time portfolio** 月度时间序列检验；近邻事件自动合并防重复计数） |
...
| **分钟因子 · 日内高频** | 分钟缓存（1m/5m/15m/30m/60m）→ 自动清洗（竞价 bar 剔除、半日市/一字板标记）→ 分钟公式求值（`m_*` 字段 + `ID_*` 聚合 / `M_*` 序列算子 + 现成高频因子 TAIL_RET/OPEN_RET/RV/JUMP_DAY/AMIHUD5/VWAP_DEV/VOLUME_CLOCK/AUC_VOL_RATIO/LIMIT_UP_TIME/OVERNIGHT_RET/INTRADAY_RET）→ 自动折叠日频面板，下游全链路复用；**时刻 IC 曲线**（09:45~14:55 各时点采截面算 RankIC，回答「因子信息何时最强」）；回测 **execute_at 执行时点**（next_close=次日收盘成交 / 尾盘=信号日收盘成交 / next_open=次日开盘成交：新仓按开→收、存量持仓按收→收、卖出按隔夜收→开计收益）；因子库「日内高频」类别预置 14 个因子，批量扫描自动分流；工作流「因子构建（分钟）」节点 |
| **策略回测** | 向量化回测、绩效指标、净值/回撤曲线（工作流回测节点 + QUBE 策略工作台）；**执行口径统一为 T 日信号 → T+1 收盘成交**（信号当日与建仓当日均不计收益，无前视）；**仅普通股票多头**（`normalize` 固定为 `long_only`，负信号视为不买入，不融资/不融券/不做空；总仓位 ≤ 100% 为循环内硬约束——停牌/一字板冻结旧仓时买入按预算缩减，缩减天数见 `leverage_summary`/assumptions）；支持可选基准对比（如 `000300.SH`）；**交易成本拆分**（佣金/滑点/印花税分列金额、占比与 bps/换手）；**容量分析**（信号在参与率约束下可容纳的资金规模，逐档资金规模给出受限天数比例与理论容量上限；ADV 窗口可调，assumptions 明示 A 股整手/涨跌停/冲击成本未模拟项）；**回测风格归因**（回测记录页一键运行：组合收益 ~ Σ β×风格因子收益 + alpha，回答「策略赚的钱来自哪种风格、剩的是不是纯 alpha」）；**参数敏感性**（网格扫描：佣金/滑点/印花税/止盈止损逐组合回测对比，验证策略稳健性）；**退市强制清算**（数据提前截止的持仓按末日价 ×(1-`delisting_loss`) 变现，`delisting_events` 明细 + assumptions 明示，杜绝「死股冻结不实现损失」）；**复牌跳空收益**（收益率前向填充口径，停牌损失如实入账） |
...
| **实验管理** | 实验记录、多实验对比、研究日志；回测与工作流运行完成后自动创建实验记录并关联 source_id |
| **风险与组合分析** | 风格暴露（Barra 精简）/ 组合优化（SLSQP 带约束）/ 绩效补充指标（alpha·beta/上下行捕获/Active Share）/ 压力测试（含**历史情景回放**：2015 股灾/2018 熊市/2024 小微盘危机等真实窗口 × 当前权重）/ **组合事前风险预测**（风格+行业因子协方差 → 预测波动 + 因子风险贡献分解）；**回测归因含行业因子**（均值偏离编码，回答「超额来自行业还是选股」）；组合优化仅允许纯多头；**块自助蒙特卡洛**（保留自相关与波动聚集）；支持**从本地行情缓存加载面板**（股票池+区间，前复权）与**回测记录风格归因**（回归法，`/api/risk/attribution-run`） |
| **数据管理** | QMT 数据下载、缓存管理、数据质量检查（仅检查行情文件并单列参考快照；含价格跳变/量能尖峰/疑似停牌退市披露）、**最近除权事件清单**（adjust_factor 跳变检测：股票/日期/比例）、财务快照拉取（含 **Cashflow 现金流表**，供现金流/质量因子；字段按别名表归一）、标的池快照（**两融/沪深股通**，仅作为市场结构参考，系统不用于做空/融资）、**主要宽基指数成分自动快照**（沪深300/中证500/中证1000/科创50/创业板指逐日积累 as-of 成分）、数据时效检查（**交易日历口径**：QMT 日历优先、工作日近似兜底，长假不误报）；**退市/历史代码清单**（维护退市代码，全市场批量下载自动并入，消除幸存者偏差）；**历史参考快照导入**（指数成分/行业/合约名称/两融池 as-of 导入，仅作研究元数据，不改变「只做普通多头」约束） |
| **工作台** | 首页 **每日研究简报**：市场状态 + **研究数据就绪度**（股票数/历史长度/参考快照/财务/分钟/QMT 连接，未达门槛明确提示「仅方法验证」）+ 因子池体检摘要 + 近 7 日除权事件 + 数据时效 + 最近批处理（/api/ops/briefing 实时聚合，15 分钟自动刷新） |
...
- **回测节点**：向量化回测，默认成本佣金率 0.001 / 滑点 0.001 / 卖出印花税 0.0005（与回测接口、QUBE 一致），T 日信号 T+1 收盘成交（信号当日与建仓当日均不计收益），输出净值/回撤曲线与完整绩效（年化收益/波动/夏普/索提诺/卡玛/最大回撤/VaR/CVaR/胜率/盈亏比/月度收益），支持可选基准对比（跟踪误差/信息比率）。**投资边界固定为普通多头**：正信号按日归一为满仓组合，总仓位 ≤ 100%；负信号视为不买入；不融资、不融券、不做空。逐仓风控（止盈/止损/移动止损）仅作用于多头持仓；停牌/一字板导致调仓无法成交时不推演挂单（持仓保持，次数在 assumptions 明示），且冻结旧仓挤占预算时买入按预算缩减，任何情况下总仓位不越界（缩减天数见 `leverage_summary`）。成本明细拆分输出（佣金/滑点/印花税分列）。
- **因子样本外验证节点**：walk-forward 滚动锚定分割（训练 [0,t) → 测试 [t, t+test) 逐折后移），输出每折 in-sample IC 与 OOS IC/分层多空统计量、聚合 OOS IC 均值/t 值/同向一致性/衰减，防过拟合（多空统计量仅供研究诊断，不代表可交易做空）。
...
- **代码沙箱**：回测/QUBE 的信号代码经 `backend/services/sandbox.py` 执行——优先用 OpenSandbox 容器隔离（数据经文件 API 以 CSV 传入，不挂载卷，兼容 Windows），Docker/服务未就绪时降级为进程内执行并标注。仅覆盖 `/api/backtest/run-strategy` 与 QUBE `run_backtest`；因子研究页与工作流因子节点保持原生执行。`GET /api/system/sandbox` 会**实际探测 opensandbox-server**（HTTP 健康探测 + TCP 兜底），不再把「包已安装/配置已开启」显示为「隔离已就绪」。
...
状态可查 `GET /api/system/sandbox`（实际探测 server，`active=false` 即当前为进程内降级）。
...
- **交易边界**：系统只做普通股票多头，不融资、不融券、不做空、不使用杠杆；`normalize` 固定为 `long_only`，`max_gross_exposure` 固定上限 1.0，负信号在回测引擎内清零。
- **严谨性约定**（自研研究/回测路径默认启用）：
  ...
  - **预置因子指标分来源**：外部抓取的参考指标标为 `external_ref`，本地 QMT 行情重算后标为 `local_recalc`；只有 `local_recalc` 且样本达到门槛（≥300 只、≥252 个交易日）的因子允许入因子池或进入组合回测；
...
- **空数据错误统一**：缓存为空/未连接时，数据类接口统一返回结构化错误（`code: no_cached_data` / `qmt_not_connected` + message + hint），不再静默返回空列表，研究员可区分「没数据」与「出 bug」。探索类接口的 SQL/字段/日期错误同样带 `code`（`invalid_query` / `invalid_request` / `no_data` / `no_cached_data`），便于程序分支。
- 前端采用 OpenCode 浅色主题风格，frontend/DESIGN-opencode.ai.md
- 工程检查：`uv run ruff check backend tests` 已纳入 CI；本地研究平台允许在“可选增强失败不应拖垮主流程”处使用宽泛异常捕获，相关规则（BLE001/S110/S112）在 pyproject.toml 中显式声明豁免。
